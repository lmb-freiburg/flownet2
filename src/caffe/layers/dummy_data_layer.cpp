...
void DummyDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (refill_[filler_id]) {
        boost::shared_ptr<Filler<Dtype> > filler = fillers_[filler_id];

        IterationFiller<Dtype>* iterFiller = dynamic_cast<IterationFiller<Dtype>*>(filler.get());
        if(iterFiller)
                iterFiller->setIteration(this->GetNet()->iter());
